...
def closest_centerline_point(car_pos, centerline):
    car_pos = np.asarray(car_pos[:2], dtype=np.float32)
    diff = centerline - car_pos
    dist2 = np.sum(diff ** 2, axis=1)
    idx = np.argmin(dist2)
    centerline_point = centerline[idx]
    distance = math.sqrt(dist2[idx])
    prev_idx = (idx - 1) % len(centerline)
    next_idx = (idx + 1) % len(centerline)
    tangent = centerline[next_idx] - centerline[prev_idx]
    tangent /= np.linalg.norm(tangent)
    offset = car_pos - centerline_point
    cross = tangent[0] * offset[1] - tangent[1] * offset[0]
    signed_distance = -math.copysign(distance, cross)

    return idx, centerline_point, signed_distance
...
def build_observation(
    ...
    dist2centerline_point_norm = normalize_unsigned(abs(dist2centerline_point), MAX_DIS2CENTERLINE)
    ...
    metrics = {
        ...
        "y": float(car_pos[1]),
        "z": float(car_pos[2]),
        "heading": float(car_heading_deg),
        "road_heading": float(road_heading_deg),
        "drift_angle": float(heading_error_deg),
        "slip_angle": float(slip_angle_deg),
        "yaw_rate": float(yaw_rate),
        "cruising_speed": float(cruising_speed),
        ...
        "dist2centerline": float(dist2centerline_point),
    }
    ...
    obs = np.concatenate([
        ...
        np.array([
            ...
            slip_angle_deg,
            heading_error_deg,
            abs(dist2centerline_point),
            yaw_rate,
        ])
    ])

    return obs.astype(np.float32), metrics
...
class VehicleEnv(gym.Env):
    def __init__(self, mode="train"):
        ...
        self.touch_rear = self.driver.getDevice("touch sensor rear")
        self.touch_rear.enable(self.timestep)
    ...
    def reset(self, seed=None, options=None):
        ...
        obs, metrics = build_observation(driver=self.driver, centerline=self.centerline, lidar=self.lidar,
                                        gyro=self.gyro,
                                 gps=self.gps, compass=self.compass,prev_action=self.prev_action)

        print(f"Reward: {int(self.reward_episode)} | Time: {round(self.num_timesteps*(self.timestep/1000), 2)}s")
        self.num_timesteps = 0
        self.reward_episode = 0
        self.num_epochs += 1
        self.resetting = True
    ...
    def step(self, action):
        ...
        obs, metrics = build_observation(driver=self.driver, centerline=self.centerline, lidar=self.lidar, gyro=self.gyro,
                                gps=self.gps, compass=self.compass, prev_action=self.prev_action)
        ...
        # info dict used by TrackingEvalCallback (training.py) to log metrics
        # during evaluation episodes. Cheap to always fill in; SB3 ignores
        # unknown info keys during normal training rollouts.
        metrics["steering_action"] = float(steering)
        metrics["throttle_action"] = float(throttle)
        metrics["sim_step"] = float(self.num_timesteps)
        metrics["sim_time"] = float(round(self.num_timesteps * (self.timestep / 1000), 3))
        print(self.num_timesteps)

        return (obs[:self.obs_size], reward, terminated, truncated, metrics)