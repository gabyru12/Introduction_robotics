...
def build_agent(name: str, keyboard=None):
    ...
    #     return DQNAgent.load("drift_open_space_40min_60_90_degrees_checkpoints/dqn_latest.zip")
    # if name == "ppo":
    #     from agents.ppo_agent import PPOAgent
    #     return PPOAgent.load("drift_open_space_40min_60_90_degrees_checkpoints/ppo_latest.zip")
    raise ValueError(f"Unknown policy: {name!r}")


def apply_action(driver: Driver, steering: float, throttle: float, brake: float, speed_ms: float = 0.0,
                 max_steering_angle = 1.0) -> None:
    ...
    driver.setSteeringAngle(steering * max_steering_angle)
...
def controll_car():
    ...
    world_name = os.path.basename(driver.getWorldPath())
    car_node = driver.getFromDef("VEHICLE")

    # Compute centerline_list "centerline" and comulative_distance_list "s"
    world_name = os.path.basename(driver.getWorldPath())
    centerline = get_centerline(world_name)
    s = compute_centerline_s(centerline)

    # Build agent with keyboard
    keyboard = None
    keyboard = driver.getKeyboard()
    keyboard.enable(time_step)
    agent = build_agent("keyboard", keyboard=keyboard)
    agent.reset()
    ...
    timesteps = 0
    previous_easier_control = False
    #last_progress = 0
    while driver.step() != -1:
        #x,y = driver.getFromDef("VEHICLE").getField("translation").getSFVec3f()[:2]
        #angular_velocity_z = driver.getFromDef("VEHICLE").getVelocity()[5]

        obs = build_observation(driver, centerline, lidar=lidar, gyro=gyro, gps=gps, compass=compass,
                                prev_action=prev_action,
                                n_lidar_bins=12)
        steering, throttle, brake = agent.act(obs=None)
        if steering > 0.7:
            steering = 0.7
        apply_action(driver, steering, throttle, 0.0, speed_ms=abs(driver.getCurrentSpeed()), max_steering_angle=max_steering_angle)
        prev_action = np.array([steering, throttle], dtype=np.float32)
        previous_easier_control = facilitate_driving(driver, previous_easier_control, world_name)

        timesteps += 1

def teleport_car_through_track_points():
    driver = Driver()
    world_name = os.path.basename(driver.getWorldPath())
    centerline = get_centerline(world_name)

    car = driver.getFromDef("VEHICLE")

    if car is None:
        raise RuntimeError(
            "Could not find vehicle DEF VEHICLE"
        )

    translation_field = car.getField("translation")

    z_height = 0.35  # adjust to your vehicle
...
def heading_of_road_at_car_position():
    driver = Driver()
    time_step = int(driver.getBasicTimeStep())
    ...
    road_heading = centerline[(idx + 1) % len(centerline)] - centerline[idx]
    road_heading = math.atan2(road_heading[1], road_heading[0])
    road_heading_unit = np.array([round(math.cos(road_heading),3), round(math.sin(road_heading),3), 0.0],
                                 dtype=np.float32)
    ...
    drift_angle = _wrap_angle(car_heading - road_heading) * 180 / math.pi
    car_heading = car_heading * 180 / math.pi
    road_heading = road_heading * 180 / math.pi
    print(f"road_heading: {road_heading} | road_heading_unit: {road_heading_unit} | car_heading: {car_heading} | "
          f"car_heading_unit:"
          f" {car_heading_unit} | "
          f"drift_angle:"
          f" {drift_angle}")
...
#teleport_car_through_track_points()
controll_car()
#rotate_car()
#heading_of_road_at_car_position()
